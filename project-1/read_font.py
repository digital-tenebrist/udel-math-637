...
from utils.plot_utils.plot_font import PlotUtils

def main():
    font_path='/data/udel-ms-data-science/math-637/project-1/char-fonts/raw'
    test_font='junk.csv'

    trf = TrimRawFonts(f'{font_path}/../trimmed/{test_font}',debug=True)
    ...
    #         tdf = trf.raw_font_as_df(rfr)
    #         if tdf is not None:
    #             print(f'df.cols {tdf}')
    #         print(f'{chr(int(rfr.ascii_label))}')
    #         trimmed = trf.raw_font_as_list(rfr)
    #         if trimmed:
    #             print(','.join([str(x) for x in trimmed]))
    #             pu = PlotUtils(trimmed)
    #             pu.show_plot()
